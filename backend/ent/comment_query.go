...
func (_q *CommentQuery) QueryAuthor() *ProfileQuery {
...
func (_q *CommentQuery) WithAuthor(opts ...func(*ProfileQuery)) *CommentQuery {
	query := (&ProfileClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *CommentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Comment, error) {
	...
	if query := _q.withAuthor; query != nil {
		if err := _q.loadAuthor(ctx, query, nodes, nil,
			func(n *Comment, e *Profile) { n.Edges.Author = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *CommentQuery) loadAuthor(ctx context.Context, query *ProfileQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Profile)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Comment)
	for i := range nodes {
		fk := nodes[i].AuthorID
