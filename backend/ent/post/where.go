...
func HasAuthorWith(preds ...predicate.Profile) predicate.Post {
	return predicate.Post(func(s *sql.Selector) {
		step := newAuthorStep()
...
func HasSavesWith(preds ...predicate.SavedPost) predicate.Post {
	return predicate.Post(func(s *sql.Selector) {
		step := newSavesStep()
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
...
func HasMediaWith(preds ...predicate.PostMedia) predicate.Post {
	return predicate.Post(func(s *sql.Selector) {
		step := newMediaStep()
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
