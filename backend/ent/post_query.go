...
type PostQuery struct {
	...
	withSaves    *SavedPostQuery
	withMedia    *PostMediaQuery
...
func (_q *PostQuery) QueryAuthor() *ProfileQuery {
...
func (_q *PostQuery) QueryLikes() *PostLikeQuery {
...
func (_q *PostQuery) QuerySaves() *SavedPostQuery {
...
func (_q *PostQuery) QueryMedia() *PostMediaQuery {
	query := (&PostMediaClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *PostQuery) WithAuthor(opts ...func(*ProfileQuery)) *PostQuery {
	query := (&ProfileClient{config: _q.config}).Query()
...
func (_q *PostQuery) WithLikes(opts ...func(*PostLikeQuery)) *PostQuery {
	query := (&PostLikeClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PostQuery) WithSaves(opts ...func(*SavedPostQuery)) *PostQuery {
	query := (&SavedPostClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PostQuery) WithMedia(opts ...func(*PostMediaQuery)) *PostQuery {
	query := (&PostMediaClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PostQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Post, error) {
	...
	if query := _q.withAuthor; query != nil {
		if err := _q.loadAuthor(ctx, query, nodes, nil,
			func(n *Post, e *Profile) { n.Edges.Author = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withSaves; query != nil {
		if err := _q.loadSaves(ctx, query, nodes,
			func(n *Post) { n.Edges.Saves = []*SavedPost{} },
			func(n *Post, e *SavedPost) { n.Edges.Saves = append(n.Edges.Saves, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withMedia; query != nil {
		if err := _q.loadMedia(ctx, query, nodes,
			func(n *Post) { n.Edges.Media = []*PostMedia{} },
			func(n *Post, e *PostMedia) { n.Edges.Media = append(n.Edges.Media, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *PostQuery) loadAuthor(ctx context.Context, query *ProfileQuery, nodes []*Post, init func(*Post), assign func(*Post, *Profile)) error {
...
func (_q *PostQuery) loadLikes(ctx context.Context, query *PostLikeQuery, nodes []*Post, init func(*Post), assign func(*Post, *PostLike)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Post)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(postlike.FieldPostID)
	}
...
func (_q *PostQuery) loadSaves(ctx context.Context, query *SavedPostQuery, nodes []*Post, init func(*Post), assign func(*Post, *SavedPost)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Post)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(savedpost.FieldPostID)
	}
	query.Where(predicate.SavedPost(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(post.SavesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *PostQuery) loadMedia(ctx context.Context, query *PostMediaQuery, nodes []*Post, init func(*Post), assign func(*Post, *PostMedia)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Post)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(postmedia.FieldPostID)
	}
	query.Where(predicate.PostMedia(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(post.MediaColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
